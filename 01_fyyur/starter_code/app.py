...
def venues():
    ...
    # Create specific cities and state combinations
    cities_states = set()
    for venue in venues:
    ...
    for temp in cities_states:
        venues_list = []
        for venue in venues:
            if (venue.city == temp[0]) and (venue.state == temp[1]):
                # sort venues by cities and states
                ...
                venues_list.append(
                    {"id": venue.id, "name": venue.name, "upcoming_shows": upcoming}
                )

        data.append({"city": temp[0], "state": temp[1], "venues": venues_list})
    # data = [
    #     {
    #         "city": "San Francisco",
    #         "state": "CA",
...
def search_venues():
    ...
    response = {"count": len(venue_search), "data": venue_list}
    # response = {
    #     "count": 1,
    #     "data": [
    #         {
...
def search_artists():
    ...
    # removes unnecessary spaces, case insensitive search which ignores letters on both sides
    search_term = request.form.get("search_term", "").strip()
    ...
    for artist in artist_search:
        ...
        artist_list.append(
            {"id": artist.id, "name": artist.name, "num_upcoming_shows": upcoming}
        )

    response = {"count": len(artist_search), "data": artist_list}
