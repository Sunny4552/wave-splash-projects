...
def create_app(test_config=None):
    ...
    def retrieve_questions():
        ...
        # assign category to category type
        categories = Category.query.all()
        categories_dict = {}
        for category in categories:
            categories_dict[category.id] = category.type
    ...
    def delete_question(id):
        # delete the question with given question id
        try:
    ...
    def new_question():
        # get data from new question forms
        ...
        try:
            # assigned submitted values as new question
            question = Question(
    ...
    def play_quiz():
        ...
        try:
            ...
            if category["type"] == "click":  # get questions based on category
                next_questions = Question.query.filter(
                    Question.id.notin_((previous_questions))
                ).all()
            else:  # get all questions that havn't been displayed
                next_questions = (
            ...
            # new questions sets up range of next questions
            new_question = (
                next_questions[random.randrange(0, len(next_questions))].format()
    ...
    def not_found(error):
        return (
            jsonify({"success": False, "error": 404, "message": "Item not found."}),
            404,
        )
    ...
    # Error handler when the server fails.
